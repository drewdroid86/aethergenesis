...
app.get('/api/catalog/search', async (req, res) => {
  // Security: Runtime type validation of query parameters to prevent array-injection
  if (req.query.name !== undefined && typeof req.query.name !== 'string') {
    return res.status(400).json({ error: 'Invalid name parameter' });
  }
  if (req.query.spectral_class !== undefined && typeof req.query.spectral_class !== 'string') {
...
app.get('/api/horizons/search', async (req, res) => {
  // Security: Runtime type validation of query parameters to prevent array-injection
  if (req.query.body_id !== undefined && typeof req.query.body_id !== 'string') {
  ...
  const limit = req.query.limit ? Math.min(50, parseInt(req.query.limit)) : 20;
