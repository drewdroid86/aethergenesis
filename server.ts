...
app.use((req, res, next) => { 
    ...
    // Security: Secure regex for local/private origins to prevent bypasses like http://10.evil.com
    const localOriginRegex = /^https?:\/\/(?:localhost|127\.0\.0\.1|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(?:1[6-9]|2\d|3[01])\.\d+\.\d+|100\.(?:6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.\d+\.\d+)(?::\d+)?$/;
    const isAllowed = origin && (
        allowedOrigins.includes(origin) || 
        (isDev && localOriginRegex.test(origin))
    );
    if (origin && isAllowed) { 
