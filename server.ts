...
app.get('/api/catalog/search', async (req, res) => {
  // Security: Runtime type validation for query parameters to prevent array injection
  const name = typeof req.query.name === 'string' ? req.query.name : undefined;
  const spectralClass = typeof req.query.spectral_class === 'string' ? req.query.spectral_class : undefined;
  
  const massMin = typeof req.query.mass_min_solar === 'string' ? parseFloat(req.query.mass_min_solar) : undefined;
  const massMax = typeof req.query.mass_max_solar === 'string' ? parseFloat(req.query.mass_max_solar) : undefined;
  const distMax = typeof req.query.distance_max_ly === 'string' ? parseFloat(req.query.distance_max_ly) : undefined;
  const limit = typeof req.query.limit === 'string' ? Math.min(20, parseInt(req.query.limit)) : 10;
...
app.get('/api/horizons/search', async (req, res) => {
  // Security: Runtime type validation for query parameters
  const bodyId = typeof req.query.body_id === 'string' ? req.query.body_id : undefined;
  const type = typeof req.query.type === 'string' ? req.query.type : undefined;
  const limit = typeof req.query.limit === 'string' ? Math.min(50, parseInt(req.query.limit)) : 20;
  ...
  if (bodyId) {
    ...
    try {
      ...
      console.error('Horizons Search Error:', err?.message || 'Unknown error');
