...
app.use((_req, res, next) => {
  ...
  const wsOrigins = allowedOrigins.map(o => {
    try {
      ...
      const baseOrigin = `${protocol}//${url.host}`;
      // Add both base origin and simulation port origin if they differ
      return url.port ? baseOrigin : `${baseOrigin} ${baseOrigin}:${wsPort}`;
    } catch {
      return '';
    }
  }).filter(Boolean).join(' ');
...
app.post('/api/analyze', async (req, res) => {
  ...
  // Security: Guard against malformed request bodies before processing (strictly exclude arrays)
