...
export function useSimulation(containerRef: React.RefObject<HTMLDivElement | null>) {
        ...
        try {
            ...
            const animate = () => {
                ...
                try {
                    ...
                    engine.update(selectedStarRef.current, isScrubbingRef.current);
                    controls.update();

                    // Update HUD
                    if (hudRefs.hudX.current) hudRefs.hudX.current.innerText = engine.camera.position.x.toFixed(1);
                    if (hudRefs.hudY.current) hudRefs.hudY.current.innerText = engine.camera.position.y.toFixed(1);
                    if (hudRefs.hudZ.current) hudRefs.hudZ.current.innerText = engine.camera.position.z.toFixed(1);
                    if (hudRefs.hudAge.current) hudRefs.hudAge.current.innerText = (13.8 + engine.appTime * 0.0001).toFixed(4);
                    ...
                    if (selectedStarRef.current) {
                        ...
                        if (uiRefs.timelineFill.current) uiRefs.timelineFill.current.style.width = `${s.t * 100}%`;
                    }
                } catch (err: any) {
    ...
    }, [isPaused]); // Restart simulation if pause state changes (or pass it to engine.update)
