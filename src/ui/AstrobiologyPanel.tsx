...
export const AstrobiologyPanel: React.FC<AstrobiologyPanelProps> = ({
  data,
  selectedStar,
  onClose,
}) => {
      ...
      <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 shadow-2xl">
        <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10 group/header">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-lg font-medium text-white/90 font-mono tracking-wider">
              Astrobiology
            </h2>
          </div>
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div
                    className={`px-2 py-0.5 rounded text-xs font-mono font-medium ${
                      planet.climateState === 'habitable'
                        ? 'bg-emerald-500/20 text-emerald-300'
                        : planet.climateState === 'snowball'
                          ? 'bg-blue-500/20 text-blue-300'
                          : planet.climateState === 'moist_greenhouse'
                ...
                <div className="space-y-2 text-xs font-mono text-white/60">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={
                        planet.surfaceTemperature_K > 273 && planet.surfaceTemperature_K < 373
                          ? 'text-emerald-400'
                          : ''
                      }
                    >
                  ...
                  <div className="flex justify-between">
                    <span className="flex items-center">
                      <Droplets className="w-3 h-3 mr-1" /> Habitability
                    </span>
                    <span>{(planet.compositeScore * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="flex items-center">
                      <ShieldAlert className="w-3 h-3 mr-1" /> Extinction Risk
                    </span>
                    <span
                      className={
                        planet.extinctionRiskLevel !== 'none' ? 'text-red-400' : 'text-emerald-400'
                      }
                    >
