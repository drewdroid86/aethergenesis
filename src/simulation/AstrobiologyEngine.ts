...
export class AstrobiologyEngine {
  private history: Map<
    string,
    { timeInHz_yr: number; highestScore: number; civEmergenceTime: number }
  > = new Map();
  ...
  public evaluatePlanet(
    ...
    const orbitalScore = isInHabitableZone
      ? 1.0
      : Math.max(0, 1.0 - Math.min(Math.abs(S_eff - 1.0), 1.0));
    ...
    const compositeScore =
      orbitalScore * thermalScore * atmosphereScore * stellarActivityScore * ageScore;

    // History tracking
    let record = this.history.get(planet_id) || {
      timeInHz_yr: 0,
      highestScore: 0,
      civEmergenceTime: 0,
    };
