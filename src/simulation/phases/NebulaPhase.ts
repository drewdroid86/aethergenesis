...
export class NebulaPhase implements PhaseComponent {
  ...
  update(
    ...
    this.dustCloud.scale.setScalar(
      1.0 - normT * STELLAR_CONSTANTS.VISUALS.NEBULA_DUST_SCALE_REDUCTION
    );
  }
  ...
  updateAsSecondary(delta: number, appTime: number, cameraPos: THREE.Vector3, normT: number): void {
    ...
    this.dustCloud.rotation.y +=
      delta * STELLAR_CONSTANTS.VISUALS.NEBULA_DUST_ROTATION_SPEED_SECONDARY;
    (this.dustCloud.material as THREE.ShaderMaterial).uniforms.uAlpha.value =
      1.0 - normT * STELLAR_CONSTANTS.VISUALS.NEBULA_DUST_ALPHA_REDUCTION;
    this.dustCloud.scale.setScalar(
      STELLAR_CONSTANTS.VISUALS.NEBULA_DUST_SCALE_SECONDARY -
        normT * STELLAR_CONSTANTS.VISUALS.NEBULA_DUST_SCALE_SECONDARY_REDUCTION
    );
  }
