...
export class ProtostarPhase implements PhaseComponent {
  ...
  update(
    ...
    const normT =
      (t - STELLAR_CONSTANTS.PHASE_BOUNDARIES.PROTOSTAR_START) /
      STELLAR_CONSTANTS.PHASE_BOUNDARIES.PROTOSTAR_DURATION;
    const introScale =
      this.baseRadius *
      (STELLAR_CONSTANTS.VISUALS.PROTOSTAR_INTRO_SCALE_MIN +
        normT * (1.0 - STELLAR_CONSTANTS.VISUALS.PROTOSTAR_INTRO_SCALE_MIN));
  ...
  setOpacity(opacity: number): void {
    ...
    (this.protostarDisk.material as THREE.MeshBasicMaterial).opacity =
      opacity * STELLAR_CONSTANTS.VISUALS.PROTOSTAR_DISK_OPACITY;
  }
