...
export class SupernovaPhase implements PhaseComponent {
  ...
  update(
    ...
    const normT =
      (t - STELLAR_CONSTANTS.PHASE_BOUNDARIES.SUPERNOVA_START) /
      STELLAR_CONSTANTS.PHASE_BOUNDARIES.SUPERNOVA_DURATION;
    ...
    if (this.mass > STELLAR_CONSTANTS.PHYSICS.MASS_THRESHOLD_SUPERNOVA) {
      ...
      this.snRing.scale.setScalar(
        (1.0 + normT * STELLAR_CONSTANTS.VISUALS.SUPERNOVA_RING_SCALE_HIGH_MASS) *
          (physics.strongForce || 1.0)
      );
      (this.snRing.material as THREE.MeshBasicMaterial).opacity = 1.0 - Math.pow(normT, 2);
      (this.snRing.material as THREE.MeshBasicMaterial).color.setHex(
        normT < 0.2 ? 0xffffff : 0xff5500
      );
      ...
      this.snRing.visible = true;
      this.snRing.scale.setScalar(
        (1.0 + normT * STELLAR_CONSTANTS.VISUALS.SUPERNOVA_RING_SCALE_LOW_MASS) *
          (physics.strongForce || 1.0)
      );
      ...
      this.ejectaMesh.visible = true;
      this.ejectaMat.uniforms.uExp.value =
        normT * STELLAR_CONSTANTS.VISUALS.SUPERNOVA_EJECTA_EXP_SPEED * (physics.strongForce || 1.0);
