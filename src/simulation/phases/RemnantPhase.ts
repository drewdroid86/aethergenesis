...
export class RemnantPhase implements PhaseComponent {
  ...
  updateRemnantOpacity(delta: number, targetNs: number): void {
    ...
    if (this._opNs !== nextOpNs) {
      this._opNs = nextOpNs;
      const nsMeshMat = (this.neutronStarGroup.children[0] as THREE.Mesh)
    ...
    const isVisible =
      this._opNs > STELLAR_CONSTANTS.TRANSITIONS.VISIBILITY_THRESHOLD ||
      this._opNsLines > STELLAR_CONSTANTS.TRANSITIONS.VISIBILITY_THRESHOLD;
