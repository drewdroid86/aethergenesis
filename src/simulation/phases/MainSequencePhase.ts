...
export class MainSequencePhase implements PhaseComponent {
  ...
  init(parent: THREE.Group): void {
    ...
    if (this.mass < 0.3)
      msColor = 0xcc44bb; // Brown dwarf — magenta
    ...
    for (let i = 0; i < 4; i++) {
      ...
      matrix.scale(
        new THREE.Vector3(s * this.baseRadius, s * this.baseRadius, s * this.baseRadius)
      );
    ...
    const hzRadius = Math.max(
      STELLAR_CONSTANTS.VISUALS.HZ_RADIUS_BASE,
      Math.sqrt(lum) * STELLAR_CONSTANTS.VISUALS.HZ_LUM_FACTOR
    );
    ...
    for (let i = 0; i < 2; i++) {
      const dist =
        hzRadius * STELLAR_CONSTANTS.VISUALS.PLANET_HZ_DIST_FACTOR + Math.random() * 8 + i * 2;
  ...
  update(
    ...
    if (cameraPos) {
      const distSq = cameraPos.distanceToSquared(
        (this as any).parent?.position ?? new THREE.Vector3()
      );
