...
export function computeMainSequenceLifetime(mass_solar: number): number {
  // BOLT: Algebraic expansion of Math.pow(x, -2.5) -> 1 / (x^2 * sqrt(x))
  return 1e10 * (1.0 / (mass_solar * mass_solar * Math.sqrt(mass_solar)));
}
...
export function computeLuminosity(mass_solar: number): number {
  if (mass_solar > 0.43) {
    // BOLT: Algebraic expansion of Math.pow(x, 4.0) -> (x^2 * x^2)
    const mSq = mass_solar * mass_solar;
    return mSq * mSq;
  }
...
export function computeRadius(
  ...
  switch (phase) {
    ...
    case 'main_sequence':
      return r_ms;
    case 'red_giant': {
      // BOLT: Avoid Math.pow for squaring
...
export function computeTemperature(
  ...
  // BOLT: Math.pow(x, 0.25) -> sqrt(sqrt(x))
...
export function computeSchwarzschild(mass_solar: number): number {
  // BOLT: Pre-calculated constant for 2GM/c^2 per solar mass in km
