...
export function computeMainSequenceLifetime(mass_solar: number): number {
  // BOLT: Optimized Math.pow(x, -2.5) as 1 / (x^2 * sqrt(x))
...
export function computeLuminosity(mass_solar: number): number {
  if (mass_solar > 0.43) {
    // BOLT: Optimized Math.pow(x, 4) as (x^2)^2
...
export function computeTemperature(
  ...
  // BOLT: Optimized Math.pow(x, 0.25) as sqrt(sqrt(x)) and cached squared radius
...
export function computeSchwarzschild(mass_solar: number): number {
  // BOLT: Simplified constant multiplication (2 * G * M_sun / c^2) / 1000
