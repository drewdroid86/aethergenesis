...
const getPhaseForT = (t: number): number => {
  ...
  if (t < 0.7) return PHASES.MAIN_SEQUENCE;
  if (t < 0.85) return PHASES.RED_GIANT;
  if (t < 0.9) return PHASES.SUPERNOVA;
...
export class HeroStarSystem extends THREE.Group {
  ...
  constructor() {
    ...
    this.hitMesh = new THREE.Mesh(GEOMETRIES.hit, new THREE.MeshBasicMaterial({ visible: false }));
  ...
  update(
    ...
    if (this.t < STELLAR_CONSTANTS.PHASE_BOUNDARIES.NEBULA_LIMIT) {
      ...
      this.currentTemp =
        STELLAR_CONSTANTS.TEMPERATURES.NEBULA_START +
        normT * STELLAR_CONSTANTS.TEMPERATURES.NEBULA_MAX;
      this.currentLum = normT * STELLAR_CONSTANTS.LUMINOSITY.NEBULA_MAX;
    } else if (this.phase === PHASES.PROTOSTAR) {
      targetProto = 1;
      const normT =
        (this.t - STELLAR_CONSTANTS.PHASE_BOUNDARIES.NEBULA_LIMIT) /
        STELLAR_CONSTANTS.PHASE_BOUNDARIES.PROTOSTAR_DURATION;
      ...
      this.currentTemp =
        STELLAR_CONSTANTS.TEMPERATURES.PROTOSTAR_START +
        normT * (this.tHeat - STELLAR_CONSTANTS.TEMPERATURES.PROTOSTAR_START);
      this.currentLum =
        normT * Math.pow(this.mass, STELLAR_CONSTANTS.PHYSICS.MASS_LUMINOSITY_EXPONENT);
