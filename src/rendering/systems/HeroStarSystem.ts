...
export class HeroStarSystem extends THREE.Group implements PhysicalBody {
    ...
    update(delta: number, appTime: number, cameraPos: THREE.Vector3, physics: PhysicsConstants, overrideT?: number, cosmicAge?: number) {
        ...
        // Update uHbar in materials
        const hbar = physics.hbar || 1.0;
        if (this.protostarMat.uniforms.uHbar) this.protostarMat.uniforms.uHbar.value = hbar;
        if (this.starMat.uniforms.uHbar) this.starMat.uniforms.uHbar.value = hbar;
        if (this.redGiantMat.uniforms.uHbar) this.redGiantMat.uniforms.uHbar.value = hbar;
