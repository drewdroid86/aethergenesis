...
} from '../simulation/StellarPhysics';
...
export class Engine {
  ...
  constructor(container: HTMLElement) {
    ...
    this._backgroundStarMat = new THREE.PointsMaterial({
      color: 0xffffff,
      size: 0.5,
      transparent: true,
      opacity: 0.8,
    ...
    this._backgroundStarGeo.setAttribute(
      'position',
  ...
  update(
      ...
      for (let i = 0; i < this.heroStars.length; i++) {
        ...
        for (let j = i + 1; j < this.heroStars.length; j++) {
          ...
          if (distSq < minDistSq && distSq > 0.01) {
            ...
            const force = ((minDist - dist) / minDist) * delta * 2;
    ...
    for (let i = 0; i < this.heroStars.length; i++) {
      ...
      if (!this.isPaused && !isScrubbing) {
        ...
        star.velocity.multiplyScalar(0.92); // Damping

        const MAX_VEL = 8.0;
        if (star.velocity.lengthSq() > MAX_VEL * MAX_VEL) {
          star.velocity.normalize().multiplyScalar(MAX_VEL);
        }
