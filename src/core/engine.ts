...
export class Engine {
    ...
    update(selectedStar: HeroStarSystem | null, isScrubbing: boolean) {
        ...
        this.heroStars.forEach(hs => {
            if (this.isPaused || (hs === selectedStar && isScrubbing)) {
                hs.update(0, this.appTime, this.camera.position);
            } else {
                hs.update(delta, this.appTime, this.camera.position);
            }
