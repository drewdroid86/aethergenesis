...
## 2025-05-27 - Hoisting Physics Constants and Gated GPU Updates
**Learning:** In the $O(N^2)$ n-body loop, even simple divisions like `target.mass / center.mass` are redundant if one mass is constant across the inner loop; hoisting the reciprocal mass saves N divisions per iteration. Similarly, orbital systems with static parameters (eccentricity, semi-major axis) should pre-calculate transformation constants in the constructor to avoid redundant per-frame `sqrt` and trig operations. In Three.js rendering, gating `instanceMatrix.needsUpdate` behind a dirty-flag or state-change check prevents expensive per-frame GPU buffer uploads for static geometry.
**Action:** Hoist invariant physics calculations out of nested loops. Pre-calculate orbital geometry constants during object initialization. Only mark instanced mesh matrices as needing update if their transformation state has actually changed.
