...
console.assert(Math.abs(sun.luminosity_solar - 1.0) < 0.1, 'Luminosity mismatch');
console.assert(Math.abs(sun.temperature_K - 5778) < 100, 'Temperature mismatch');
console.assert(sun.spectralClass.startsWith('G'), 'Spectral class mismatch');
...
console.assert(massive.luminosity_solar > 10000, 'Luminosity too low');
console.assert(massive.spectralClass.startsWith('O') || massive.spectralClass.startsWith('B'), 'Spectral class mismatch');
...
console.assert(advanced.state.phase === 'red_giant', 'Phase transition mismatch');
...
console.assert(postSN.state.remnantType === 'black_hole', 'Remnant type mismatch');
console.assert(postSN.state.schwarzschildRadius_km && postSN.state.schwarzschildRadius_km > 0, 'Invalid Schwarzschild radius');
