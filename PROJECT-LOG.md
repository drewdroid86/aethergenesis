...
### 2026-09-01 — Stellar Phase & Thermodynamic State Synchronization via StellarPhysics (Gemini)
- **Physical Phase & Property Derivation (`src/rendering/systems/HeroStarSystem.ts`):** Removed hardcoded timeline fraction gates (`getPhaseForT`, `t < 0.05` nebula checks) and static piecewise temperature/luminosity formulas. `HeroStarSystem.phase`, `currentTemp`, and `currentLum` are now evaluated authentically via `createStellarState(this.physicsId, this.mass, metallicity, age_yr)` given the star's physical age ($\text{age\_yr} = \text{currentRealAge} \times 10^6$), mass, and metallicity. Low-mass M-dwarfs ($0.22\,M_\odot$ at $9.568\,\text{Gyr}$) now correctly evaluate to `main_sequence` with physical $T \approx 3070\,\text{K}$ and $L \approx 0.0071\,L_\odot$ (fixing the issue where they erroneously remained in cold $480\,\text{K}$ nebula state for 22 Gyr).
- **Inspect Timeline Slider Decoupling:** Maintained $t \in [0, 1]$ as the normalized lifespan scrubber representing progress across the star's total lifetime $\tau_{\text{MS}}$, while driving physical state derivation through physical age without altering $\tau_{\text{MS}}$.
- **Stellar Physics Longevity Test Assertions (`scripts/verify-physics.ts`, `scripts/e2e/f4_comet.test.ts`):** Added assertions validating that a $0.22\,M_\odot$ star at $9.568\,\text{Gyr}$ is in `main_sequence` with expected main sequence temperature ($2500 < T < 4000\,\text{K}$) and luminosity ($0.001 < L < 0.1\,L_\odot$).
- **Verification:** `npm run typecheck`, `npm run build` (2.37s), `npm run lint` (0 warnings, 0 errors), `npm test` (47/47 E2E tests passing across `f1_presets`, `f2_ui_modes`, `f3_galaxy`, `f4_comet`), `npm run test:mcp`, and `scripts/verify-physics.ts` all passed.
