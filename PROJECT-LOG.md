...
- **Weighted Geometric Mean Composite Scoring & Astrobiology Keying (`src/simulation/AstrobiologyEngine.ts`, `src/simulation/SimulationCoordinator.ts`):** Transitioned `compositeScore` to a weighted geometric mean with smooth exponential decay outside HZ and broader spectral class sensitivity (M, A, O, B, red giant, agb). Added static `AstrobiologyEngine.key(starId, planetId)` for canonical star-scoped keying and prefix-based cache invalidation in `clearHistory()`.
- **E2E Test Coverage & Standalone Verification (`scripts/e2e/f4_comet.test.ts`, `scripts/verify-astrobiology.ts`):** Added test `F4-T4-51` verifying configuration schema, hysteresis transitions, and created `scripts/verify-astrobiology.ts` covering the complete biogenesis lifecycle from habitability through Kardashev Tier 3 emergence.
- **Verification:** `npm run typecheck`, `npm run build` (1.76s), `npm run lint` (0 errors, 0 warnings), `npm test` (48/48 E2E tests passing across all 4 suites), `npm run verify:physics`, and `scripts/verify-astrobiology.ts` passed cleanly.
...
- **Physical Phase & Thermodynamic State Sync:**
  - $1.51\,M_\odot$ at $0.94\,\text{Gyr} \rightarrow \text{main\_sequence}$, $T \approx 7398\,\text{K}$, $L \approx 5.20\,L_\odot$ (hot, luminous F-type star, resolving leftover nebula states).
  - $0.16\,M_\odot$ at $5\text{--}13.8\,\text{Gyr} \rightarrow \text{main\_sequence}$, $T \approx 2903\,\text{K}$, $L \approx 0.0034\,L_\odot$ (cool dwarf, prevented from prematurely becoming a Red Giant on the cosmic clock during terminal timeline scrub).
  - $0.22\,M_\odot$ at $9.57\,\text{Gyr} \rightarrow \text{main\_sequence}$, $T \approx 3070\,\text{K}$, $L \approx 0.0071\,L_\odot$.
- **Test Suite Updates (`scripts/verify-physics.ts`):** Added explicit assertions verifying all 3 target astrophysical cases ($1.51\,M_\odot$ F-star, $0.16\,M_\odot$ cool dwarf longevity, $0.22\,M_\odot$ M-dwarf).
- **Verification:** `npm run typecheck`, `npm run build` (2.02s), `npm run lint` (0 warnings, 0 errors), `npm test` (47/47 E2E tests passing across all suites), `npm run test:mcp`, and `scripts/verify-physics.ts` passed.
...
### 2026-08-26 — Kroupa IMF Mass Sampling & Steady-State Stellar Recycling (Gemini 3.7 Flash)
- **Kroupa Initial Mass Function (`src/simulation/StellarPhysics.ts`):** Implemented `sampleKroupaMass()` with literature citation (Kroupa 2001) using inverse transform sampling on a piecewise power law (76.2% low-mass dwarf branch $0.08–0.5\,M_\odot$ with $\alpha=1.3$, 23.8% intermediate/massive branch $0.5–20\,M_\odot$ with $\alpha=2.3$). Replaces flat uniform mass rolls that previously biased 68.7% of stars to $M > 1.4\,M_\odot$ ($\tau < 4.3\,\text{Gyr}$).
- **Phase-Proportional Initial State & Stellar Recycling (`src/rendering/systems/HeroStarSystem.ts`):** Initialized stars across active lifecycle stages (~76% Main Sequence, ~10% Red Giant, ~8% Nebula/Protostar, ~2% Supernova, ~4% Remnant) and derived `birthAge = cosmicAge - (t * lifespan / 1000) / effG`, eliminating the "born dead" startup issue. Added `recycleToNewGeneration()` which triggers when non-inspected remnants reach $t \ge 1.15$, recycling ejected stellar matter back into new molecular clouds (`PHASES.NEBULA`, $t = 0.0$) with fresh `birthAge = cosmicAge` to maintain a dynamic steady-state galactic population.
- **Two-Way Scrubber & Preset Birth Age Synchronization (`src/utils/hooks/useStarSelection.ts`, `src/utils/hooks/useSimulation.ts`, `src/rendering/systems/HeroStarSystem.ts`):** Added `syncBirthAge()` on `onScrubEnd` and inside `applyPreset()`, synchronizing `birthAge` with `cosmicAge` so user scrub positions and catalog presets persist rather than snapping back to Remnant on the next frame.
- **Verification:** `npm run verify:physics` (passed citation and tsc checks), `npm run typecheck`, `npm run build` (1.69s), and `npm run lint` passed with 0 errors and 0 warnings. `npm run benchmark:physics` achieved 0.855ms/tick.
